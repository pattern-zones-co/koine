...
describe.skipIf(!RUN_E2E_TESTS)("Real Claude CLI (E2E)", () => {
  ...
  it("can execute a simple prompt with real CLI", { timeout: 60_000 }, async () => {
  ...
  it("can use system prompt with real CLI", { timeout: 60_000 }, async () => {
    const result = await executeClaudeCli({
  ...
  it("returns usage statistics from real CLI", { timeout: 60_000 }, async () => {
    const result = await executeClaudeCli({
      prompt: "Say hello",
      timeoutMs: 30_000,
    });
  ...
  it("returns session ID for conversation continuity", { timeout: 60_000 }, async () => {
    const result = await executeClaudeCli({
...
describe.skipIf(!RUN_E2E_TESTS)("Real CLI Error Handling (E2E)", () => {
  it("handles timeout gracefully", { timeout: 10_000 }, async () => {
